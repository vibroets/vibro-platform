...
const BulkAssignTaskScreen: React.FC = () => {
  ...
  const parseFormData = useCallback(async () => {
    try {
      ...
      if (!parsedResult) {
        setError('Form data missing or invalid. Please go back and try again.');
        return;
      }
      ...
      setCurrentStageId(currentStageId || null);

      // Restore saved selections using stage-specific key (persists per-stage until submission)
      const stageConfigKey = `bulk_assignments_${formId}_${currentStageId}`;
      let restored = false;
      try {
        const savedConfigString = await SecureStoreService.get(stageConfigKey);
      ...
      if (!currentStage) {
        setError('Current stage not found');
        return;
      }

      // Parse parent questions with follow-up tasks for the current stage only
      let questionsWithFollowUp: ParentQuestionWithFollowUp[] = [];
      const collectAllQuestionsWithFollowUp = (stage: Stage) => {
        const results: ParentQuestionWithFollowUp[] = [];

        stage.questions?.forEach((question: Question) => {
          // Check if this question has follow-up tasks in its logic
          ...
          // If this question has follow-up tasks in its logic, add it to the list
          if (hasFollowUpTask) {
            results.push({
      ...
      if (currentFormValues && visibleQuestions) {
        ...
        questionsWithFollowUp = currentStage.questions
        ...
        questionsWithFollowUp = collectAllQuestionsWithFollowUp(currentStage);
      }

      // Also include mobile-created follow-up tasks for questions that only exist in temporary state.
      if (Array.isArray(temporaryFollowUpTasks) && temporaryFollowUpTasks.length > 0) {
        const questionMap = new Map(
          questionsWithFollowUp.map((question) => [question.question_uuid, question]),
        );

        temporaryFollowUpTasks.forEach((task: any) => {
          const parentQuestionUuid = task?.parentQuestionUuid;
          if (!parentQuestionUuid || questionMap.has(parentQuestionUuid)) {
            return;
          }

          const parentQuestion = currentStage.questions?.find(
            (question: Question) => question.question_uuid === parentQuestionUuid,
          );

          if (parentQuestion) {
            questionMap.set(parentQuestionUuid, {
              question_uuid: parentQuestion.question_uuid,
        ...
        questionsWithFollowUp = Array.from(questionMap.values());
      }
  ...
  const handleSaveAssignments = useCallback(async () => {
    ...
    try {
      ...
      // Save to secure store with stage-specific key
      const stageConfigKey = `bulk_assignments_${formId}_${currentStageId}`;
      await SecureStoreService.set(stageConfigKey, JSON.stringify(assignmentConfig));
  ...
  }, [selectedQuestions, selectedUsers, selectedGroups, selectedStages, formId, currentStageId]);
  ...
  return (
    <View style={styles.container}>
      ...
      {parentQuestions.length > 0 && (
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder="Search questions or stages..."
            value={searchQuery}
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              {hasNonAssignedFollowUpOnly
                ...
                : 'This form has no follow-up tasks configured.'}
            </Text>
          </View>
        ) : (
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => { setShowUserModal(false); setUserSearchQuery(""); }}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Select Users</Text>
              <TouchableOpacity onPress={() => { setShowUserModal(false); setUserSearchQuery(""); }}>
            ...
            <FlatList
              ...
              style={{ maxHeight: 300 }}
            />
            <TouchableOpacity style={styles.modalDoneBtn} onPress={() => { setShowUserModal(false); setUserSearchQuery(""); }}>
      ...
      <Modal
        ...
        transparent
        animationType="fade"
        onRequestClose={() => { setShowGroupModal(false); setGroupSearchQuery(""); }}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Select Groups</Text>
              <TouchableOpacity onPress={() => { setShowGroupModal(false); setGroupSearchQuery(""); }}>
                <MaterialIcons name="close" size={20} color="#666" />
              </TouchableOpacity>
            </View>
            <View style={styles.modalSearchRow}>
            ...
            <FlatList
              ...
              style={{ maxHeight: 300 }}
            />
            <TouchableOpacity style={styles.modalDoneBtn} onPress={() => { setShowGroupModal(false); setGroupSearchQuery(""); }}>
              <Text style={styles.modalDoneText}>Done</Text>
            </TouchableOpacity>
          </View>
        </View>
...
export default BulkAssignTaskScreen;
