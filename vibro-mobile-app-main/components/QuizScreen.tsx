...
const getQuizProgressKey = (scheduleId: any, contentId: any, contentType: string) => `quiz_progress_sched_${scheduleId != null ? scheduleId : 'direct'}_${contentType}_${contentId}`;
...
export default function QuizScreen({ item, scheduleId, onBack, onQuizComplete }: QuizScreenProps) {
  ...
  useEffect(() => {
    if (item.id && item.type) {
      AsyncStorage.getItem(getVideoProgressKey(scheduleId, item.id, item.type)).then((val: string | null) => {
        if (val) {
  ...
  useEffect(() => {
    ...
    if (item.id && item.type) {
      AsyncStorage.getItem(getQuizProgressKey(scheduleId, item.id, item.type)).then((val: string | null) => {
        if (val) {
          try {
  ...
  useEffect(() => {
    ...
    if (item.id && item.type && shuffledQuestions.length > 0) {
      AsyncStorage.setItem(getQuizProgressKey(scheduleId, item.id, item.type), JSON.stringify({
  ...
  const doSubmit = async () => {
    ...
    if (item.id && item.type) {
      AsyncStorage.removeItem(getQuizProgressKey(scheduleId, item.id, item.type));
    }
      ...
      if (parsed.type === "progress") {
        ...
        progressSaveTimer.current = setTimeout(() => {
          if (item.id && item.type && parsed.progress < 95) {
            AsyncStorage.setItem(getVideoProgressKey(scheduleId, item.id, item.type), String(parsed.progress));
          }
        }, 500);
        if (parsed.progress >= 95 && !quizUnlocked) {
          setQuizUnlocked(true);
          if (item.id && item.type) AsyncStorage.removeItem(getVideoProgressKey(scheduleId, item.id, item.type));
