...
interface ReopenSubmissionModalProps {
  visible: boolean;
  onClose: () => void;
  taskId: string;
  submissionId: string;
...
const ReopenSubmissionModal: React.FC<ReopenSubmissionModalProps> = ({
  visible,
  onClose,
  taskId,
  submissionId,
  ...
  const handleReopen = useCallback(async () => {
    if (!reason.trim()) {
      Toast.show({
        type: 'error',
        text1: 'Required',
        text2: 'Please provide a reason for reopening',
        position: 'top'
      });
      return;
    }
    ...
    if (selectedUsers.length === 0 && selectedGroups.length === 0) {
      Toast.show({
        type: 'error',
        text1: 'Required',
        text2: 'Please select at least one user or group to reassign the task to',
        position: 'top'
      });
      return;
    }
    ...
    setSubmitting(true);
    try {
      // Call the reopen endpoint with reason and selected users/groups
      ...
      const totalAssignees = selectedUsers.length + selectedGroups.length;
      Toast.show({
        type: 'success',
        text1: 'Task Reopened & Reassigned',
        text2: `Task reassigned to ${totalAssignees} user(s)/group(s)`,
        position: 'top'
      });
      ...
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: error?.response?.data?.message || 'Failed to reopen task',
        position: 'top'
      });
    } finally {
      setSubmitting(false);
    }
  }, [taskId, reason, selectedUsers, selectedGroups, onReopenSuccess, onClose]);
      ...
      <View style={styles.overlay}>
        <View style={styles.modal}>
          <View style={styles.header}>
            <Text style={styles.title}>Reopen Followup Task</Text>
          ...
          <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Reason for Reopening *</Text>
              <TextInput
                style={styles.reasonInput}
                value={reason}
                onChangeText={setReason}
                placeholder="Please explain why this followup task needs to be reopened..."
                multiline
                numberOfLines={3}
            ...
            {/* Assignment Section */}
            <View style={styles.section}>
              <Text style={styles.sectionTitle}>Reassign to Users/Groups</Text>
          ...
          <View style={styles.footer}>
            ...
            <TouchableOpacity
              style={[styles.button, styles.reopenButton, submitting && styles.disabledButton]}
